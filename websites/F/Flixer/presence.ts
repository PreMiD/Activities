...
const ActivityAssets = {
  Logo: 'https://i.imgur.com/ElDXOQr.png',
}
...
async function fetchTMDBData(id: string, isTV: boolean = false, retries = 3): Promise<any> {
...
function getVideoInfo() {
  try {
    ...
    const currentTime = video.currentTime && !isNaN(video.currentTime) ? video.currentTime : 0
...
function getVideoQuality(video: HTMLVideoElement): string | null {
  ...
  for (const selector of qualitySelectors) {
    ...
    if (element) {
      ...
      const qualityMatch = text?.match(/(\d+p|\d+K|4K|HD|FHD|UHD)/i)
      if (qualityMatch) {
...
function extractSeasonEpisode(text: string) {
  if (!text) return { title: text, season: null, episode: null }
  
  const patterns = [
    /(.+?)\s+S(\d+)\s+E(?:pisode\s+)?(\d+)/i,
    /(.+?)\s+S(\d+)E(\d+)/i,
    /(.+?)\s+Season\s+(\d+)\s+Episode\s+(\d+)/i,
    /(.+?)\s+(\d+)x(\d+)/i,
    /(.+?)\s+Ep(?:isode)?\s+(\d+)/i,
    /(.+?)\s+S(\d+)\s+Episode\s+(\d+)/i,
  ]
      ...
      for (const selector of infoSelectors) {
        ...
        if (element?.textContent) {
          ...
          if (!episode) {
            ...
            if (epMatch) episode = `E${epMatch[1] || epMatch[2]}`
          }
          if (!season) {
            const seasonMatch = text.match(/Season\s*(\d+)|S(\d+)/i)
            if (seasonMatch) season = `S${seasonMatch[1] || seasonMatch[2]}`
          }
        }
      }
    }
