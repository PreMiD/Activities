...
presence.on('UpdateData', async () => {
  ...
  if (pathname.startsWith('/movie/') || pathname.startsWith('/tv/')) {
    const title =
      document.querySelector('img[alt][src*="tmdb.org"]')?.getAttribute('alt') ||
      document.title.replace(' | FlickyStream', '') ||
      'Viewing Movie';

    const image =
      document.querySelector('div.hidden.md\\:block img')?.getAttribute('src') ||
      document.querySelector('img[src*="tmdb.org"]')?.getAttribute('src');
  ...
  if (pathname.startsWith('/player/movie/')) {
    const title =
      document.querySelector('h1.text-xl.md\\:text-2xl.font-semibold')?.textContent?.trim() ||
      document.title.replace(' | FlickyStream', '') ||
      'Viewing Movie';
  ...
  if (pathname.startsWith('/player/tv/')) {
    ...
    const [, seasonNo, episodeNo] =
      episodePart?.match(/Season\s+(\d+)\s+Episode\s+(\d+)/) || [];
  ...
  if (pathname.includes('/search')) {
    ...
    presenceData.state = `Query: ${decodeURIComponent(
