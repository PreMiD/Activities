...
presence.on("iFrameData", (data: unknown) => {
  ...
  if (iframeData.iFrameVideoData) {
    playback = true;
    currentTime = iframeData.iFrameVideoData.currTime;
    duration = iframeData.iFrameVideoData.dur;
    paused = iframeData.iFrameVideoData.paused;
  }
});
...
function getPageTitle(): string {
  ...
  if (path.startsWith('/infantil/')) {
    ...
    if (clanTitle && !clanTitle.includes("Dibujos animados") && clanTitle.length > 10) {
      return clanTitle;
    }
  }
  ...
  if (path.startsWith('/eltiempo/') || path === '/eltiempo') {
    ...
    if (weatherTitle && weatherTitle.length > 10) {
      return weatherTitle.substring(0, 60) + (weatherTitle.length > 60 ? "..." : "");
    }
  }

  if (path.includes('/receta-') || path.includes('/cocina/') || path.includes('/television/cocina/')) {
  ...
  if (path.includes('/play/audios/')) {
    const audioTitle = document.querySelector('[class*="title"]')?.textContent?.trim();
  ...
  return document.title.replace(" - RTVE.es", "").replace(" | RTVE", "").replace(" - RTVE", "").trim();
}

function getCategory(): string {
  const breadcrumb = document.querySelector(".breadcrumb, nav[aria-label='Ruta de navegación']");
  if (breadcrumb) {
...
function getProgramInfo(): { title: string; subtitle: string; description: string } {
  const title = document.querySelector("h1")?.textContent?.trim() || "";
  const subtitle = document.querySelector("h2, .subtitle, .program-info")?.textContent?.trim() || "";
  const description = document.querySelector('meta[property="og:description"]')?.getAttribute("content") || "";

  return { title, subtitle, description };
}

function getSectionInfo(): { sectionName: string; defaultTitle: string } {
  const path = document.location.pathname.toLowerCase();
  ...
  else if (path.startsWith('/play/radio/') || path.startsWith('/play/audios/')) {
    return {
...
function getLogoToUse(): ActivityAssets {
  ...
  if (path.startsWith('/play/') || path === '/play' ||
    path.startsWith('/playz/') || path === '/playz') {
...
presence.on("UpdateData", async () => {
  const [buttons, showTimestamps, privacy, showCover, detailedInfo] = await Promise.all([
  ...
  presenceSettings = { buttons, showTimestamps, privacy, showCover, detailedInfo };
  ...
  const presenceData: PresenceData = {
    largeImageKey: presenceSettings.showCover ? getLogoToUse() : ActivityAssets.RTVE,
    startTimestamp: browsingTimestamp
  };

  const video = document.querySelector("video");
  const audio = document.querySelector("audio");
  const isPlayingVideo = video && !isNaN(video.duration);
  const isPlayingAudio = audio && !isNaN(audio.duration);
  ...
  if (pathname.includes("/videos/directo/")) {
    if (privacy) {
      ...
      if (programInfo.subtitle) presenceData.state += ` - ${programInfo.subtitle}`;
    }
    ...
    if (privacy) {
      ...
      presenceData.state = programInfo.title || pageTitle || sectionInfo.defaultTitle;
    ...
    if (presenceSettings.buttons && !privacy) {
      ...
      if (category.includes("Serie") || programInfo.description.includes("serie")) {
        const seriesLink = document.querySelector('a[href*="/series/"], a[href*="/programas/"]');
        if (seriesLink) {
    ...
    if (privacy) {
      ...
      presenceData.details = strings.searching;
      const searchTerm = searchParams.get("q") || document.querySelector('input[type="search"]')?.getAttribute("value");
    ...
    if (privacy) {
      ...
      if (pathname.includes("/series/") || pathname.includes("/programas/")) {
        ...
        const seriesName = pathname.split('/').pop()?.replace(/-/g, ' ') || "Series";
        presenceData.state = seriesName.charAt(0).toUpperCase() + seriesName.slice(1);
      }
      else {
        presenceData.state = pageTitle || sectionInfo.defaultTitle;
        if (category && presenceSettings.detailedInfo && category.length < 30) {
          presenceData.state += ` | ${category}`;
        }
      }
