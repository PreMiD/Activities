...
/**
 * Tower Defense map names formatted correctly for status
 */
const defenseMaps: Record<DefenseData["map"], string> = {
  "meadow": "Sunny Meadow"
};
 ...
 */
type DefenseData = {
  corrects: Record<string, any>;
  incorrects: Record<string, any>;
  defenseId: string;
  dmg: number;
  health: number;
  hwGoal: any | null;
 ...
 * @returns HTMLElement
 */
const getDivFromClass = (t: string) => {
  return Array.from(document.querySelectorAll("div")).find(e => e.className.includes(t));
};

/**
 * Converts MM:SS to seconds. (e.g. 12:34 -> 754)
 * 
 * @param ts string (e.g. 12:34)
 * @returns number of seconds (754)
 */
const convertTimeStringToSeconds = (ts: string) => {
  if (!ts.includes(":") || ts.split(":").length != 2) throw Error("Invalid time string");

  return (parseInt(ts.split(":")[0] as string) * 60) + (parseInt(ts.split(":")[1] as string));
};
  ...
  switch (host) {
    ...
    case "help.blooket.com":
      ...
      if (path == "/hc/") {
        ...
        if (!privacyMode) presenceData.buttons = [{
          "label": "View Article",
    ...
    case "play.blooket.com":
      if (path == "/play") {
        ...
        if (!privacyMode) presenceData.state = "Entering Game ID";
      } else if (path == "/solo") {
    ...
    case "dashboard.blooket.com":
      ...
      if (path == "/my-sets") {
        ...
        let setName: string = document.title.substring(0, document.title.length - 10);
        ...
        let userProfile: string = path.split("/")[2] as string;
    ...
    case "cocoa.blooket.com":
      // new blooket modes
      var codename: string = host.split(".")[0] as string;
      ...
      if (path.endsWith("/host")) {
        ...
        const playerCount: number = parseInt(playerCountText.textContent || "0");
    ...
    case "classic.blooket.com":
      ...
      var codename: string = host.split(".")[0] as string;
      presenceData.details = `${path.includes("host") ? "Hosting" : "Playing"} ${codenameToFullName[codename]}`;
      presenceData.largeImageKey = presenceImages[codename];
