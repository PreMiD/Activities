...
/**
 * Səhifə başlığından anime adı və bölüm məlumatını çıxarır.
 * Başlıq formatı: "Anime Adı — BÖLÜM 1 • Bölüm Adı - Tengoku Fansub"
 */
function parseTitle(): { anime: string; episode: string | null } {
  const raw = document.title.replace(" - Tengoku Fansub", "").trim();
  // "Anime Adı — BÖLÜM 3" formatında
...
presence.on("UpdateData", async () => {
  ...
  const episodeMatch = pathname.match(/^\/anime\/([^/]+)\/(\d[\d.]*)/);
  if (episodeMatch) {
