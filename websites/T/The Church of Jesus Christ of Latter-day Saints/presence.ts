import { ActivityType, Assets, timestampFromFormat, getTimestamps } from 'premid'
...
enum ActivityAssets {
  Logo = 'https://i.imgur.com/98GSvcP.png',
  OldTestament = 'https://www.churchofjesuschrist.org/imgs/6de4c15152c1c0e56b9ac5004e62d80cd04ad02d/0,0,3000,3000/max/0/default.jpeg',
  NewTestament = 'https://www.churchofjesuschrist.org/imgs/7d175abca40ddfa795593e4f713a44489acc6cd5/0,0,3000,3000/max/0/default.jpeg',
  BookOfMormon = 'https://www.churchofjesuschrist.org/imgs/59fa03a8250ea7aea58e9f3515031ea47b6ab7eb/0,0,3000,3000/max/0/default.jpeg',
  DoctrineAndCovenants = 'https://www.churchofjesuschrist.org/imgs/d9930401562cdd688233134c5f20a5d75b968b14/0,0,3000,3000/max/0/default.jpeg',
  PearlOfGreatPrice = 'https://www.churchofjesuschrist.org/imgs/16cf3d24af3de0ca15a4f6d7de71a8e870616fb9/0,0,3000,3000/max/0/default.jpeg',
}
...
let browseLocation: string | undefined = undefined;

// Reading state variables
let readingLocation: string | undefined = undefined;
let readingState: string | undefined = undefined;

// Listening state variables
let listeningLocation: string | undefined = undefined;
let listeningTitle: string | undefined = undefined;
let listeningAlbum: string | undefined = undefined;
let listeningStyle: string | undefined = undefined;

function setBrowsing(location: string, additional?: string) {
  // If we are already browsing this location, do nothing
  if (currentState == 'browsing' && browseLocation == location) return;
  ...
  let browsingTitle: string | undefined = undefined;
  if (location == 'my-home') {
    ...
    if (additional != undefined) {
      browsingTitle = `${additional} conference talks`;
    } else {
...
function setReading(location: string, additional?: string) {
// Parse the scripture reference
  ...
  if (currentState == 'reading' && location == readingLocation && readingState == previousReadingState) return;
  const previousState: string = currentState;
  ...
  let readingTitle: string | undefined = undefined;
  let button1Title: string | undefined = undefined;
  ...
  if (location == 'old-testament') {
    ...
    if (additional != undefined) {
      let year: string | undefined = location.split('-')[2];
      let month: string | undefined = location.split('-')[3];
      if (year != undefined && month != undefined) {
        button2Title = `Read Conference ${additional}`;
  ...
  let readingImage: string | undefined = undefined;
  if (location == 'old-testament') {
...
function setListening(location: string, additional?: string) {
  // Fetch the song title
  let title = parsePlayerTitle();
  if (title == undefined) {
    return; // we can't set listening state without a title
  }
  let previousTitle: string | undefined = listeningTitle;
  ...
  let previousAlbum: string | undefined = listeningAlbum;
  ...
  let previousStyle: string | undefined = listeningStyle;
  listeningStyle = style;

  // Attempt to parse the start and end timestamps
  let startTimestamp: number | undefined = parsePlayerTimestamp1();
  let endTimestamp: number | undefined = parsePlayerTimestamp2();
  if (startTimestamp != undefined && endTimestamp != undefined) {
    let timestamps = getTimestamps(startTimestamp, endTimestamp);
  ...
  let isPlaying: boolean = parsePlayerIsPlaying();
  if (isPlaying) {
  ...
  if (currentState == 'listening' && listeningLocation == location &&
      ...
      listeningAlbum == previousAlbum) return;
...
function parseScriptureReference(): string | undefined {
  ...
  let contentTitle = document.getElementsByClassName("contentTitle-JbPZw")[0];
  if (contentTitle != null) {
    if (contentTitle.children.length > 0) {
      ...
      let reference = contentTitle.children[0]?.textContent?.trim();
  ...
  if (title != null) return title.textContent?.trim();
...
function parseTalkAuthor(): string | undefined {
  ...
  let authorName = document.getElementsByClassName("author-name")[0];
  if (authorName != null) {
    author = authorName.textContent?.trim();

    // Attempt to parse the author's calling, too
    let authorCalling = document.getElementsByClassName("author-role")[0];
    if (authorCalling != null) {
      let calling = authorCalling.textContent?.trim();
...
function parsePlayerTitle(): string | undefined {
  ...
  let playerCard = document.querySelectorAll('[class*="AudioPlayerCard__TitleAndOptions"]')[0];
  if (playerCard != undefined) {
    // If it has children, we assume it's a dual title (e.g., "Book of Mormon" and "1 Nephi 1:1")
    if (playerCard.children.length > 0) {
      let title = playerCard.children[0]?.textContent?.trim();
      if (title != undefined) {
        return title;
      } else {
        return undefined;
...
function parsePlayerTimestamp1(): number | undefined {
  let elapsedTime = document.querySelectorAll('[class*="TimeBar__Elapsed"]')[0];
  if (elapsedTime != undefined) {
    let timeText = elapsedTime.textContent?.trim();
...
function parsePlayerTimestamp2(): number | undefined {
  let elapsedTime = document.querySelectorAll('[class*="TimeBar__Duration"]')[0];
  if (elapsedTime != undefined) {
    let timeText = elapsedTime.textContent?.trim();
    if (timeText != undefined) {
      return timestampFromFormat(timeText);
...
function parsePlayerIsPlaying(): boolean {
  ...
  let controls = document.querySelectorAll('[class*="Controls__PlaybackWrapper"]')[0];
  if (controls != undefined && controls.children.length > 1) {
    let playControl = controls.children[1];
...
function parsePlayerThumbnail(): string | undefined {
  ...
  let thumbnail = document.querySelectorAll('[class*="AudioPlayerCard__Thumbnail"] img')[0];
...
function parsePlayerStyle(): string | undefined {
  ...
  let select = document.querySelectorAll('[class*="AudioPlayerCard__StyledSelect"]')[0];
...
presence.on('UpdateData', async () => {
  let previousState: string = currentState;
    ...
    // Libraries
      ...
      if (path.length > 2) {
        ...
        if (path[2] == 'scriptures') {
          ...
          if (path.length > 4) {
            const year: number = parseInt(path[3] || '');
            const month: number = parseInt(path[4] || '') - 1; // Months are 0-indexed in JavaScript
