...
const endpointRegexes = {
  profileRegexp: /\/profiles\/([a-zA-Z0-9]+)/gi,
  contestsRegexp: /\/contests\/(.+)/gi,
  courseRegexp: /\/courses\/([a-zA-Z0-9]+)/gi,
}
...
presence.on('UpdateData', async () => {
  ...
  if (endpointRegexes.contestsRegexp.test(pathname)) {
    ...
    if (contestName) {
      const registrantMatch = document.querySelector('[class="bp5-button contest-registration-card__item"]')?.textContent?.trim().match(/([0-9]+)/g)?.at(-1);
      ...
      if (pathname.endsWith('scoreboard')) {
        const contestantCount = document.querySelector('.pagination__helper-text')?.textContent?.trim().match(/([0-9]+) results/)?.at(-1);
