...
enum ActivityAssets {
  logo = 'https://openani.me/favicon512_padding.png',
}
...
presence.on('UpdateData', async () => {
  ...
  try {
    ...
    if (announcerText) {
      ...
      if (jsonData.details) presenceData.details = jsonData.details;

      if (jsonData.state) presenceData.state = jsonData.state;
      ...
      if (jsonData.smallImageText) presenceData.smallImageText = jsonData.smallImageText;

      if (jsonData.buttons && currentPage !== "https://openani.me/") presenceData.buttons = jsonData.buttons;
    }
  } catch (error) {
