...
// ---- iFrame video data ----
type VideoData = { currentTime: number; duration: number; paused: boolean };
...
async function fetchAniListCover(searchTitle: string): Promise<string | null> {
  if (anilistCoverCache.has(searchTitle)) return anilistCoverCache.get(searchTitle)!;
  if (anilistInFlight.has(searchTitle)) return anilistInFlight.get(searchTitle)!;
