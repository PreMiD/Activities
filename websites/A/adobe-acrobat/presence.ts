...
const isDocumentPage = (pathname: string): boolean =>
  pathname.includes('/id/') || pathname.includes('/document/')
...
function parseBookString(raw: string): { title: string | null; author: string | null } {
  ...
  if (openIdx !== -1) {
    ...
    author = raw.slice(openIdx + 1, closeIdx).trim()
    title  = raw.slice(0, openIdx).trim()
    if (title.endsWith('.pdf')) title = title.slice(0, -4).trim()
  } else {
    author = null
    const pdfIdx = raw.indexOf('.pdf')
...
const parseDocumentTitle = () => parseBookString(document.title?.trim() ?? '')
...
const fetchBookData = async (): Promise<void> => {
  ...
  if (!isDocumentPage(pathname)) return
  ...
  try {
    ...
    console.log(`[premid-adobe] query: ${query}`)
    console.log(`[premid-adobe] best match: "${bestBook?.title}" (similarity: ${bestScore.toFixed(2)})`)
...
function applyPlaceholders(
  ...
  for (const [key, val] of Object.entries(vars))
    if (val !== undefined) result = result.split(`%${key}`).join(String(val))
  return result.trim()
}
