...
const checkAd = false
// checkPlayingAd function to determine if an ad is playing
function isAdPlaying() {
  const disabledControlElement = document.querySelectorAll<HTMLButtonElement>(
    'button:not([disabled])',
  )

  // If there is an ad element, an ad is playing
  return !!disabledControlElement.length
}
