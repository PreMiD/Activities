...
enum ActivityAssets {
  ...
  Thumbnail = 'https://raw.githubusercontent.com/AlFarrizi-Studio/Activities/add-1flex-activity/websites/0-9/1Flex/assets/thumbnail.png',
}
...
function hasCloseControl(root: ParentNode): boolean {
  return (Array.from(root.querySelectorAll('button, [role="button"], a')) as HTMLElement[])
    .some((element) => {
...
function isCloseControl(element: HTMLElement): boolean {
  ...
  const hasIcon = Boolean(element.querySelector('svg, path'))
...
function isOpenDialogCandidate(element: HTMLElement): boolean {
  ...
  const hasMedia = Boolean(element.querySelector('video, iframe, canvas, img'))
...
function getLiveTvChannelTitle(): string | undefined {
  ...
  const nowPlayingCard = (Array.from(document.querySelectorAll('div, section, article')) as HTMLElement[])
  ...
  return (Array.from(nowPlayingCard.querySelectorAll('h1, h2, h3, h4')) as HTMLElement[])
...
function getPrimaryButtonText(root: ParentNode): string | undefined {
  ...
  for (const button of buttons) {
    ...
    const text = getText(button.querySelector('span') ?? button)
  ...
  return buttons
    ...
    .map(button => getText(button.querySelector('span') ?? button))
...
function getTorrentResultCount(): number | undefined {
  ...
  const resultElements = Array.from(
    document.querySelectorAll('a[href*="magnet:"], a[href*=".torrent"], article, li, [class*="torrent"], [class*="result"]'),
  ) as HTMLElement[]
...
function getMyListCounts(): { movies: number, series: number } {
  const cards = Array.from(
    document.querySelectorAll('a[href], article, [class*="card"], [class*="movie"], [class*="show"], [class*="series"]'),
  ) as HTMLElement[]
  const seen = new Set<string>()
  ...
  for (const card of cards) {
    ...
    if (card.closest('nav, header, footer'))
      continue
...
function getUsefulHeading(root: ParentNode = document): string | undefined {
  ...
  const headings = Array.from(root.querySelectorAll('h1, h2, [class*="title"]')) as HTMLElement[]
...
function getVisibleImageLabel(root: ParentNode = document): string | undefined {
  return (Array.from(root.querySelectorAll('img')) as HTMLImageElement[])
...
function installRouteChangeHooks(): void {
  const hookFlag = '__premid1FlexRouteHooksInstalled'
  const windowWithFlag = window as unknown as Record<string, boolean>

  if (windowWithFlag[hookFlag])
    return

  windowWithFlag[hookFlag] = true
...
function getDialogScore(element: HTMLElement): number {
  ...
  if (element.querySelector('video, iframe, canvas'))
    score += 25
...
function getDialogRoot(): HTMLElement | undefined {
  const candidates = Array.from(new Set([
    ...
    ...Array.from(document.querySelectorAll('body > div div, body > div section, body > div article'))
      .filter((element) => {
        const htmlElement = element as HTMLElement
        const rect = htmlElement.getBoundingClientRect()
...
function hasTrailerPreviewFrame(root: ParentNode = document): boolean {
  return (Array.from(root.querySelectorAll('iframe')) as HTMLIFrameElement[])
  ...
  for (const key of watchHistoryKeys) {
    ...
    try {
      ...
      for (const { key, value } of values) {
        if (isRecord(value)) {
          entries.push({
            id: typeof value.id === 'number' || typeof value.id === 'string'
              ? value.id
              : key,
            ...value,
...
function getVisibleText(root?: ParentNode): string | undefined {
  ...
  const walker = document.createTreeWalker(container, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (!parent || parent.closest('script, style, noscript, template, svg'))
...
function getSelectedEpisodeText(root?: ParentNode): string | undefined {
  ...
  const candidates = Array.from(
    container.querySelectorAll('[aria-selected="true"], [aria-current="true"], [class*="selected"], [class*="bg-red"], button'),
  ) as HTMLElement[]
  ...
  const matches = candidates
    ...
    .map((element) => {
      ...
      const title = (Array.from(
        element.querySelectorAll('h2, h3, h4, [class*="title"], [class*="Title"]'),
      ) as HTMLElement[])
        .map(candidate => getText(candidate))
      ...
      const overview = (Array.from(element.querySelectorAll('p')) as HTMLElement[])
        .map(candidate => getText(candidate))
        .find(candidate => Boolean(
          candidate
          && candidate.length >= 30
