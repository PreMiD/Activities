...
function parseToSeconds(time?: string | undefined | null): number | undefined {
  ...
  const nums: number[] = parts.map(p => parseInt(p, 10));
...
presence.on('UpdateData', async () => {
  ...
  const artists = document.querySelector<HTMLElement>('#footer-track-artists')?.textContent ;
