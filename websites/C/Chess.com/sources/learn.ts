...
const learnResolver: Resolver = {
  ...
  getState: (t, doc) => {
    ...
    if (path.includes('/learn-the-openings')) return t.learn_openings;
    if (path.includes('/all-lessons')) return t.learn_all_lessons;
    ...
    if (ogTitle) {
        ...
        if (content && !content.includes('Chess.com')) return content;
    }

    return t.browsing;
  },

  getLargeImageKey: (t) => ActivityAssets.Logo, // Le pion vert (plus propre)
  getSmallImageKey: (t) => ActivityAssets.Lessons, // Le chapeau
