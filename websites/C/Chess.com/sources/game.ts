...
import { ActivityAssets, getPlayerData, formatMatch, hasPlayerControls, getGameMode } from '../util/index.js';
...
const getGameStatus = (doc: Document): GameStatus => {
    const path = doc.location.pathname;

    // 1. Cas spécifiques basés sur l'URL
    if (path.includes('/games/view/')) return GameStatus.Archive;
    if (path.includes('/play/online/watch')) return GameStatus.Spectating;
    if (path.includes('/daily')) return GameStatus.Daily;

    // 2. Détection des éléments DOM
    ...
    // 3. Logique de statut unifiée
    if (path.includes('/play/online') || path.includes('/live')) {
        // Cas du Lobby : Pas de contrôle, pas d'horloge, pas d'adversaire
        if (!hasControls && !hasClock && !topNode) return GameStatus.Lobby;
    }

    if (hasControls) return GameStatus.Playing;
    
    if (doc.querySelector('.game-over-message-component, .game-result-component')) return GameStatus.Finished;
    
    // Si on a un adversaire visible mais pas de contrôles, on regarde
    if (topName || hasClock) return GameStatus.Spectating;
    
    // Cas spécifique : recherche d'adversaire (topNode existe (placeholder) mais pas de nom)
    if (topNode && !topName) return GameStatus.Searching;

    return GameStatus.Loading;
};

const gameResolver: Resolver = {
  // isActive couvre maintenant les URLs de game.ts ET play.ts
  isActive: (pathname) => {
      ...
      // On exclut variants qui a son propre resolver
      return (isGame || isPlay) && !pathname.includes('/variants');
  },
  ...
  getDetails: (t, doc) => {
    ...
    switch (status) {
        ...
        case GameStatus.Spectating:
            if (doc.location.pathname.includes('/game/live/')) return t.watching_replay;
  ...
  getState: (t, doc, displayFormat, hideRating) => {
    ...
    if (status === GameStatus.Archive) {
        if (bottom.name && top.name) {
            return `${bottom.name} vs ${top.name}`; 
        }
  ...
  getSmallImageKey: (t, doc) => {
      ...
      if (mode === 'bullet') return ActivityAssets.Bullet;
      if (mode === 'blitz') return ActivityAssets.Blitz;
      if (mode === 'rapid') return ActivityAssets.Rapid;
      if (mode === 'daily') return ActivityAssets.Daily;
  ...
  getSmallImageText: (t, doc) => {
      const status = getGameStatus(doc);
      const mode = getGameMode(doc) || 'Live';
