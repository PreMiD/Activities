...
const puzzleResolver: Resolver = {
  ...
  getDetails: (t, doc) => {
    if (doc.location.pathname.includes('/rush')) {
        ...
        if (icon3Min) return `${t.puzzle_rush} (3 min)`;
        if (icon5Min) return `${t.puzzle_rush} (5 min)`;
        return `${t.puzzle_rush} (Survival)`;
    }
  ...
  getState: (t, doc) => {
    if (doc.location.pathname.includes('/rush')) {
        if (doc.querySelector('[data-cy="startSession"]')) return 'Menu';
    ...
    if (doc.location.pathname.includes('/battle')) {
        ...
        if (myScoreEl && opponentScoreEl) {
            ...
            return `Score: ${myScore} - ${opponentScore}`;
        }
    ...
    if (rating) {
        if (level) return `Level ${level} • ${rating}`;
  ...
  getLargeImageKey: (t, doc) => {
    if (doc.location.pathname.includes('/rush')) return ActivityAssets.PuzzleRush;
    if (doc.location.pathname.includes('/battle')) return ActivityAssets.PuzzleRush;
