...
const getMainVideo = (doc: Document): HTMLVideoElement | null => {
...
const isPlaying = (doc: Document, video: HTMLVideoElement): boolean => {
...
const videoResolver: Resolver = {
  ...
  getState: (t, doc) => {
    if (doc.location.pathname.includes('/player/')) {
        ...
        if (ogTitle) {
            ...
            if (content) return content.replace(' - Chess.com', '').trim();
        }
  ...
  getButtons: (t, doc) => {
    if (doc.location.pathname.includes('/player/')) {
        return [{ label: 'Watch Video', url: doc.location.href }];
    }
    return undefined;
  },
  ...
  getSmallImageText: (t, doc) => {
      if (doc.location.pathname.includes('/player/')) {
          ...
          if (video && !isPlaying(doc, video)) return t.pause;
          return t.play;
      }
      return undefined;
  }
