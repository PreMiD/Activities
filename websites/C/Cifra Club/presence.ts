...
presence.on("UpdateData", async (): Promise<void> => {
  try {
     ...
     */
    const songTitleElement: HTMLElement | null =
      document.querySelector<HTMLElement>(".t1");
    const artistNameElement: HTMLElement | null =
      document.querySelector<HTMLElement>(".t3");

    /**
     * Extracted song title and artist name from the DOM nodes.
     */
    const songTitle: string | null =
      songTitleElement?.textContent?.trim() ?? null;
    const artistName: string | null =
      artistNameElement?.textContent?.trim() ?? null;
