...
presence.on('iFrameData', (data: VideoState) => {
  if (data.referrer) {
    try {
      ...
      // Если referrer (страница, где iframe) не совпадает с текущим URL по пути — игнорируем
      // Это фильтрует "зависшие" iframe с предыдущих страниц в SPA
      // ВАЖНО: Некоторые сайты режут referrer до origin. Проверяем также совпадение только по origin.
      if (refUrl.origin !== currentUrl.origin)
        return

      const normalize = (p: string) => p.replace(/\/$/, '')

      // Если referrer содержит путь, проверяем его. Если только корень — пропускаем (политика безопасности).
      if (refUrl.pathname !== '/' && normalize(refUrl.pathname) !== normalize(currentUrl.pathname)) {
        // console.log(`[YummyAnime] Referrer mismatch: ${refUrl.pathname} vs ${currentUrl.pathname}`)
        // return // Пока просто логируем, чтобы не ломать playback
      }
    }
    catch {
  ...
  if (data.paused === false) {
    // console.log('[YummyAnime] Received playing state from iframe', data)
  }
  else {
    // console.log('[YummyAnime] Received paused state from iframe', data)
  }

  // Всегда обновляем состояние, если пришли данные
  iframeVideo = {
...
function getActiveEpisode(pathname: string, search: string): string {
  ...
  // New site version specific (user provided structure)
  // section.c5.c6 .oT .sB [data-selected="1"]
  const newSiteVersion = document.querySelector('section.c5.c6 .oT .sB [data-selected="1"]')
  if (newSiteVersion) {
  ...
  // Fallback for new site (site.yummyani.me) - dynamic content

  // 1. Look for specific containers found in JS bundle or common patterns
  const containers = [
    '.episodes-container',
    '.episodes-list',
    '.series-list',
  ...
  for (const selector of containers) {
    ...
    if (container) {
      const active = container.querySelector('.active, .selected, .current, [class*="active"], [class*="selected"]')
      if (active) {
  ...
  // 2. Look for any active element with a number (broad search)
  // Filtering by short text length prevents false positives from long descriptions
  ...
  for (const el of allActive) {
    ...
    // Check for exact number or "Ep X" pattern
    if (/^\d+$/.test(text))
      return text

    const m = text.match(/^(?:series|ep|episode|серия|выпуск)?\s*(\d+)\s*(?:series|ep|episode|серия|выпуск)?$/i)
    if (m?.[1])
      return m[1]
  }
...
/** URL для largeImageKey: только нормализация протокола (без смены формата — webp мог 404). */
...
function applyPosterImage(presenceData: Record<string, unknown>): void {
  ...
  if (tryAssign(document.querySelector('div.poster-block img') as HTMLImageElement))
    return
  ...
  if (tryAssign(document.querySelector('div[class*="d4"] img') as HTMLImageElement))
    return

  if (tryAssign(document.querySelector('[class*="d4"] img') as HTMLImageElement))
    return

  if (tryAssign(document.querySelector('main div[class*="d4"] img') as HTMLImageElement))
    return

  if (tryAssign(document.querySelector('img[itemprop="image"]') as HTMLImageElement))
    return
  ...
  for (const metaSel of [
    ...
    const content = document.querySelector(metaSel)?.getAttribute('content')?.trim()
...
function isPlayerBlockInView(): boolean {
  const selectors = ['#video', '[class*="Player"]', '[class*="player"]', 'video']
