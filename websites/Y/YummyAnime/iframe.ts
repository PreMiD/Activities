...
/**
 * В контексте iframe плеера не фильтруем по размеру: реклама редко, а HLS иногда даёт 0×0 rect до отрисовки.
 */
function pickBestVideo(): HTMLVideoElement | null {
  ...
  // console.log(`[YummyAnime] Found ${videos.length} videos in iframe`)
  ...
  if (withDuration.length > 0) {
    // console.log(`[YummyAnime] Found ${withDuration.length} videos with duration`)
  }
  else {
    // console.log('[YummyAnime] No videos with duration found')
  }

  // Если нет видео с известной длительностью, ищем любое играющее
...
function sendVideoState(): void {
  try {
    ...
    // console.log(`[YummyAnime] Sending video state. Duration: ${duration} (${isFiniteDuration}), Paused: ${video.paused}, CurrentTime: ${video.currentTime}`)
