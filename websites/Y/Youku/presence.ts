...
function scheduleUpdate() {
  ...
  updateTimer = window.setTimeout(() => {
    presence.setActivity(); // Force update
  }, 1000); // Update after 1 second
}

presence.on('UpdateData', async () => {
  const [privacy, showTimestamp, showButtons, showProgress, use24HourTime, showVideoDetails, compactMode] = await Promise.all([
  ...
  if (isValidVideo) {
    ...
    if (showVideoDetails && !privacy && !isPaused) {
      ...
      if (speed && speed !== '1x') details.push(speed);
      if (details.length > 0) {
    ...
    if (showTimestamp) data.startTimestamp = browsingTimestamp;
  }
